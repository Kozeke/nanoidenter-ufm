...
const Dashboard = () => {
  ...
  const sendCurveRequest = useCallback(() => {
    if (socketRef.current && socketRef.current.readyState === WebSocket.OPEN) {
      // Reset state based on curveId presence
      const resetState = {
        xMin: Infinity,
        xMax: -Infinity,
        yMin: Infinity,
      ...
      const requestData = {
        ...
        filters: { regular: regularFilters, cp_filters: cpFilters },
        ...(curveId && { curve_id: curveId }),
      };
      socketRef.current.send(JSON.stringify(requestData));
    }
  }, [curveId, numCurves, regularFilters, cpFilters]); // Dependencies for sendCurveRequest
  ...
  }, [sendCurveRequest]); // Added sendCurveRequest as dependency since it’s used in onopen
