...
const ForceIndentationSingle = ({ forceData, domainRange }) => {
  ...
  const chartOptions = {
    ...
    series: forceData.map((curve) => ({
      ...
      itemStyle: String(curve.curve_id).endsWith("_hertz")
        ? {
            color: "yellow", // Solid yellow for hertz curves
          }
        : {
            // Radial gradient for non-hertz curves
            color: {
              type: "radial",
            ...
            // Dynamic opacity based on x-axis position (optional, can remove if gradient suffices)
            opacity: (data) => {
              const xValue = data[0];
              const distanceFromCenter = Math.abs(xValue - xCenter);
              const normalizedDistance = distanceFromCenter / (xRange / 2);
              return 0.3 + 0.5 * normalizedDistance; // Kept for consistency, but gradient overrides
            },
          },
    })),
