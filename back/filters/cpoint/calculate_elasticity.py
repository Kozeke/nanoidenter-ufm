...
def calc_elspectra(
    ...
    if interp:
        yi = interp1d(x, y, fill_value="extrapolate")
        max_x = np.max(x)
        min_x = max(np.min(x), 1e-9)  # Ensure min_x is at least 1e-9
    ...
    if tip_geometry == 'sphere':
        ...
        return None  # Unsupported tip geometry
    ...
    if len(yy) <= win:
        return None  # Not enough data for filtering
